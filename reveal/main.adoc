...
-----
D:\Gamedev\blackmass\ue4\BlackMass\Source\Player\Runtime\Private\Components\BMPlayerCameraTargetComponent.h

UCLASS()
class PLAYER_TEST_API UBMPlayerCameraTargetComponent : 
...
public:
	UBMPlayerCameraTargetComponent();
...
D:\Gamedev\blackmass\ue4\BlackMass\Source\Player\Test\Private\BMPlayerCameraTargetComponentTest.cpp

IMPLEMENT_MODULE_TEST(FAttachedToPlayer_GettingAttachmentInfo_ReturnsCameraRootWithSocketName)
{
	ATBMPlayerCameraTargetOwner* Player = BMPlayerCameraTargetComponentTestHelpers::CreateTestActor(CreateWorld());
	
	// Given a camera target component attacher to a player with attachment socket name set
	UBMPlayerCameraTargetComponent* CameraTargetComponent = Player->CameraTargetComponent;
	CameraTargetComponent->SetSocketName(BMPlayerCameraTargetComponentTestHelpers::SocketName);

	// When Getting attachment info
	const FBCCameraAttachmentInfo AttachmentInfo = CameraTargetComponent->GetCameraAttachmentInfo();

	// Then the component should be the CameraRoot component
	TestTrue(TEXT("FBMCameraAttachmentInfo.Component is the set CameraRootComponent"), 
		AttachmentInfo.Component == Player->GetRootComponent());

	// And the socket name is the one set
	TestEqual(TEXT("FBMCameraAttachmentInfo.SocketName is the same as the one set"), 
		AttachmentInfo.SocketName, BMPlayerCameraTargetComponentTestHelpers::SocketName);
